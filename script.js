...
if (heroSection && heroGlow && !reduceMotion) {
  ...
  heroSection.addEventListener('mouseleave', () => {
    ...
    setTimeout(() => { inside = false; }, 10);
  });
}

/* ============================================================
   Mockup iframe scaling
   Each iframe is rendered at 1280px wide then scaled down to
   fit its container, so the preview looks like a real desktop site.
   ============================================================ */
function scaleMockupIframes() {
  document.querySelectorAll('.mockup__iframe').forEach(iframe => {
    const screen = iframe.closest('.mockup__screen');
    if (!screen) return;
    const scale = screen.offsetWidth / 1280;
    iframe.style.transform = `scale(${scale})`;
    // Expand the iframe height inversely so the clipped area fills the screen
    iframe.style.height = Math.ceil(screen.offsetHeight / scale) + 'px';
  });
}

// Run once on load and again on any resize
scaleMockupIframes();
window.addEventListener('resize', scaleMockupIframes, { passive: true });
